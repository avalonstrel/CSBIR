...
class TUBerlinData(Dataset):
	...
	def build(self, cates):
		...
		if self.mode == 'std':
			for c in cates['source']:
				...
				# for valid and test to compute MAP
				self.cate_num[c] = len(phos[c])

				random.shuffle(skts[c])
				self.source_train_skts[c] = skts[c][:-15]
				self.source_valid_skts[c] = skts[c][-15:-10]
	...
	def __getitem__(self, index):
		...
		if self.phase == 'train':
			...
			skt = self.to_tensor(self.randomflip(self.randomcrop(Image.open(fskt))))
		else:
			skt, cskt = None, -1
